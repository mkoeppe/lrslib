 ...
 */
/* Copyright (C) 1995,1999, David Avis   avis@cs.mcgill.ca      */
...
static void free_lrs_dic (lrs_dic * P);
...
***************************************************/
  ...
  if (!lrs_mp_init (ZERO, stdin, stdout))	/* initialize digits */
    return 1;
/*********************************************************************************/
...
/* print the linearities first */
  ...
  fprintf (lrs_ofp, ": %ld rows were redundant", m - nredund);
  fprintf (lrs_ofp, "\n*Number of pivots after initialization: %ld", Q->count[2]);
...
/*  print ray only if lex min, geometric option, or lp only */

  if (Q->geometric || lexmin (P, Q, col) || Q->lponly)
...
lrs_init (char *name)
{
  ...
  lrs_digits = DEC2DIG (DEFAULT_DIGITS);
  lrs_record_digits = 0;
  lrs_global_count = 0;
...
void 
lrs_close (lrs_dat * Q, char *name)
{
  if (Q->verbose && strcmp (Q->fname, "") != 0)
    fprintf (lrs_ofp, "\n*End of processing for %s ", Q->fname);
...
lrs_alloc_dat (char *name)
{
  ...
  lrs_global_count++;
/*  Q->name=strdup(name);  FUKUDA000224*/
  Q->name=(char *) malloc((strlen(name)+1)*sizeof(char));
  strcpy(Q->name,name); /* FUKUDA000224*/
...
/* process input file */
  ...
  if (fscanf (lrs_ifp, "%ld %ld %s", &Q->inputm, &Q->n, name) == EOF)
    {
      fprintf (lrs_ofp, "\nNo data in file");
      return (FALSE);
    }
	  ...
	  if (Q->hull)
	      ...
	      for (j = 0; j <= d; j++)
		{
		  if (readrat (A[0][j], oD[j]))
		    {
		      reduce (A[0][j], oD[j]);
		      lcm (Q->objlcm, oD[j]);	/* update lcm of denominators */
		    }
		  prat ("", A[0][j], oD[j]);
		  if (!Q->maximize)
		    changesign (A[0][j]);
		}
	      storesign (Q->objlcm, POS);
	      if (greater (Q->objlcm, mpone))
		for (j = 0; j <= d; j++)
		  {
		    mulint (Q->objlcm, A[0][j], A[0][j]);	/*remove denominators */
		    copy (Temp, A[0][j]);
...
/* assign local variables to structures */
  ...
  if (Q->nlinearity > 0 && Q->nonnegative)
    Q->nonnegative = FALSE;	/* not implemented with linearity */
  ...
  if (Q->maximize || Q->minimize)
    {
      ...
      if (!Q->lponly)		/* check to see if objective is dual degenerate */
	{
	  j = 1;
...
/* assign local variables to structures */
  ...
  if (Q->printcobasis)
    if (lexflag || (count[2] == (count[2] / Q->frequency) * Q->frequency))
      lrs_printcobasis (P, Q, ZERO);

  if (!lexflag && !Q->allbases && !Q->lponly)	/* not lexmin, and not printing forced */
    return FALSE;

  /* if nonnegative, decision variables are recovered from lower half of dictionary */
  ...
  if (redcol == n)
    {
      ++count[0];
      /*print cobasis if printcobasis=TRUE and count[2] a multiple of frequency */
      if (Q->printcobasis && count[2] == (count[2] / Q->frequency) * Q->frequency)
  ...
  for (i = 0; i < d; i++)
    {
      ...
      if ((!hull || Q->voronoi) && !(col == ZERO) && (rflag == temparray[i]))
	fprintf (lrs_ofp, "*");
    }
  pmp (" det=", P->det);
...
/* local assignments */
  ...
  if (hull)
    {
      fprintf (lrs_ofp, "\n*facets=%ld bases=%ld", count[0], count[2]);
	...
	fprintf (lrs_ofp, " linearities=%ld", nredundcol - homogeneous);
      fprintf (lrs_ofp, " output rows=%ld",nredundcol-homogeneous+count[0]);
    }
  else
    {
      fprintf (lrs_ofp, "\n*vertices=%ld  rays=%ld  bases=%ld", count[1], count[0], count[2]);
      if (nredundcol > 0)
	fprintf (lrs_ofp, " linearities=%ld", nredundcol);
      fprintf (lrs_ofp, " output rows=%ld",nredundcol+count[0]+count[1]);

    }				/* end of else */

  if (Q->maxdepth < MAXD)
    fprintf (lrs_ofp, "\n*Tree evaluation truncated at depth %ld", Q->maxdepth);
  ...
  if (count[0] > 1 && !Q->homogeneous)	/* print warning  */
    {
      if (hull)
	{
	  if (!Q->polytope)
...
{
  long r, s;
  long lastrow;			/* last row to be pivoted on */
  long i, j;
  lrs_mp Ns, Nt, Ars;
/* assign local variables to structures */
  lrs_mp_matrix A = P->A;
...
/* if nonnegative constraints, no need to pivot last d rows of A */
...
/*temporary: try to get new start after linearity */
    ...
    }				/* end of dual lp for feasibility */
  return (TRUE);
...
/* assign local variables to structures */
  ...
  if (Q->lponly)
	...
	storesign (A[0][0], POS);
      ...
      mulint (P->det, Q->objlcm, denom);
      prat ("\n*Objective function has value ", A[0][0], denom);
    }
  ...
  if (dict_limit > 1)
    {
      ...
      pushQ (global, (*D_p)->m, (*D_p)->d);	/* Make a new, blank spot at the end of the 
						   queue to copy into */
...
*/
  memcpy (dest->A[0][0], (global->Qtail->prev)->A[0][0],
          (d + 1) * (lrs_digits + 1) * (m + 1) * sizeof (long));
...
void 
pushQ (lrs_dat * global, long m, long d)
{
...
new_lrs_dic (long m, long d)
{
  ...
  p->A=lrs_alloc_mp_matrix(m,d);

  return p;
}

static void 
free_lrs_dic (lrs_dic * P)
{
...
/* gmp variabales cannot be cleared using free */

  for (i = 0; i < m + 1; i++)
   { 
#ifdef GMP
      for (j=0; j < d+1; j++)
        lrs_clear_mp(P->A[i][j]);
#else
      free (P->A[i]);
#endif
   }    
...
/* so don't try: free (P->det);                                           */
  ...
  lrs_clear_mp (P->det);          /* applies to GMP only */
...
/*  initialize array to zero   */
  ...
  Q->inequality[0] = 2L;
/* gmp fix 
  Q->Gcd = CALLOC ((m + 1), sizeof (lrs_mp *));
  for (j = 0; j < m + 1; j++)
    Q->Gcd[j] = CALLOC (1, sizeof (lrs_mp));

  Q->Lcm = CALLOC ((m + 1), sizeof (lrs_mp *));
  for (j = 0; j < m + 1; j++)
    Q->Lcm[j] = CALLOC (1, sizeof (lrs_mp));
*/
